    ...
    pub async fn resolve_read_args_traced(
        ...
        match (
            ...
            (Some(workspace), Some(project)) => self
                .lookup_existing(workspace, project, required)
                .await
            ...
            (None, project) => {
                let (scope, source) = self
                    .resolve_current_or_project_traced(project, actor)
                    .await?;
                let scope = self.guard(scope, required, trimmed_opt(project)).await?;
                Ok((scope, source))
            }
        }
    }
...
mod tests {
    ...
    /// V68's shape changes under them.
    fn grant_row(
        db: &std::path::Path,
    ...
    async fn traced_read_resolution_reports_where_the_scope_came_from() {
        ...
        for case in cases {
            let mut resolver = ScopeResolver::new(&store.reader, default_ws, default_proj, None);
