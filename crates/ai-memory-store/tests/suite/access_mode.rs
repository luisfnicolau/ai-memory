...
async fn fixture() -> Fixture {
    ...
    store
        .writer
        .grant_memory(reader, project, GrantRole::Reader, None)
        .await
        .unwrap();
    store
        .writer
        .grant_memory(writer, project, GrantRole::Writer, None)
        .await
        .unwrap();
    Fixture {
        _tmp: tmp,
...
async fn every_caller_against_both_modes() {
    ...
    for mode in [AccessMode::Open, AccessMode::Restricted] {
        ...
        for (who, user, read, write) in [
            ("reader grant", f.reader, true, open),
            ("writer grant", f.writer, true, true),
            ("no grant", f.outsider, open, open),
        ] {
            let reads = f
                .store
                .reader
                .access_for(user, f.project, GrantRole::Reader)
                .await
                .unwrap();
            let writes = f
                .store
                .reader
                .access_for(user, f.project, GrantRole::Writer)
                .await
                .unwrap();
...
async fn new_projects_follow_the_server_default_and_admit_their_creator() {
    ...
    assert!(matches!(
        f.store
            .reader
            .access_for(f.reader, open_one, GrantRole::Writer)
            .await
            .unwrap(),
        Access::Granted
    ));
    ...
    assert!(
        matches!(
            f.store
                .reader
                .access_for(f.outsider, locked, GrantRole::Writer)
                .await
                .unwrap(),
            Access::Granted
        ),
    ...
    assert!(
        matches!(
            f.store
                .reader
                .access_for(f.reader, locked, GrantRole::Reader)
                .await
                .unwrap(),
            Access::Denied(_)
        ),
    ...
    assert!(
        matches!(
            f.store
                .reader
                .access_for(f.reader, plain, GrantRole::Reader)
                .await
                .unwrap(),
            Access::Denied(_)
        ),
    ...
    for reserved in [
        ...
        assert!(
            matches!(
                f.store
                    .reader
                    .access_for(f.reader, id, GrantRole::Writer)
                    .await
                    .unwrap(),
                Access::Granted
            ),
